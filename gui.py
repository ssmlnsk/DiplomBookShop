...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.page_req = self.ui.stackedWidget_req
        self.page_req_2 = self.ui.stackedWidget_req_2
        self.page_prov = self.ui.stackedWidget_prov
        self.page_id = [0]
        self.now_page = 0
        self.block_req = True
        ...
        self.ui.btn_all_clients_2.clicked.connect(self.page_all_clients_2)
        self.ui.btn_all_req.clicked.connect(self.page_all_requests)
        self.ui.btn_all_pr_od.clicked.connect(self.page_all_pr_od)
        self.ui.btn_all_req_2.clicked.connect(self.page_all_requests_2)
        self.ui.btn_all_prov.clicked.connect(self.page_all_providers)
        ...
        self.ui.btn_back_ord.clicked.connect(self.page_ord_back)
        self.ui.btn_back_ord_2.clicked.connect(self.page_ord_back)

        self.ui.btn_back_ord_3.clicked.connect(self.page_ord_back_2)
        self.ui.btn_back_ord_4.clicked.connect(self.page_ord_back_2)
        ...
        self.ui.btn_back_prov.clicked.connect(self.page_prov_back)
        self.ui.btn_back_prov_2.clicked.connect(self.page_prov_back)
    ...
    def page_ord_back(self):
        self.page_req.setCurrentIndex(0)
        logging.log(logging.INFO, 'Переход к таблице "Клиент"')

    def page_ord_back_2(self):
        self.page_req_2.setCurrentIndex(0)
        logging.log(logging.INFO, 'Переход к таблице "Клиент"')

    def page_prov_back(self):
        self.page_prov.setCurrentIndex(0)
        logging.log(logging.INFO, 'Переход к таблице "Клиент"')
    ...
    def page_all_clients(self):
        ...
        self.page_req.setCurrentIndex(2)
        logging.log(logging.INFO, 'Переход к таблице "Клиент"')

    def page_all_clients_2(self):
        """
        Переход к странице с таблицей клиентов.
        """
        self.updateTableClient_2()
        self.page_req_2.setCurrentIndex(2)
        logging.log(logging.INFO, 'Переход к таблице "Клиент"')
    ...
    def page_all_pr_od(self):
        ...
        """
        self.updateTableProviderOrder()
        self.page_prov.setCurrentIndex(2)
    ...
    def page_all_requests(self):
        ...
        """
        self.updateTableOrder()
        self.page_req.setCurrentIndex(1)
        logging.log(logging.INFO, 'Переход к таблице "Заказ"')

    def page_all_requests_2(self):
        """
        Переход к странице с таблицей заказов.
        """
        self.updateTableOrder()
        self.page_req_2.setCurrentIndex(1)
        logging.log(logging.INFO, 'Переход к таблице "Заказ"')

    def page_all_providers(self):
        """
        Переход к странице с таблицей заказов.
        """
        self.updateTableProvider()
        self.page_prov.setCurrentIndex(1)
        logging.log(logging.INFO, 'Переход к таблице "Поставщик"')
    ...
    def updateTableClient_2(self):
        """
        Обновление таблицы `Клиент`.
        """
        self.table_clients_2.clear()
        rec = self.facade.read_clients()
        self.ui.table_clients_2.setColumnCount(6)
        self.ui.table_clients_2.setRowCount(len(rec))
        self.ui.table_clients_2.setHorizontalHeaderLabels(
            ['Код клиента', 'Фамилия', 'Имя', 'Отчество', 'Дата рождения', 'e-mail'])
        ...
        for i, client in enumerate(rec):
            for x, field in enumerate(client):
                ...
                if x == 0:
                    item.setFlags(Qt.ItemIsEnabled)
                self.ui.table_clients_2.setItem(i, x, item)
        logging.log(logging.INFO, 'Таблица "Клиент" обновлена')
    ...
    def updateTableHistory(self):
        ...
        self.table_entry.setColumnCount(4)
        self.table_entry.setRowCount(len(rec))
        self.table_entry.setHorizontalHeaderLabels(['Дата входа', 'Дата выхода', 'Блокировка', 'Логин сотрудника'])
    ...
    def updateTableProvider(self):
        ...
        self.table_provider.setHorizontalHeaderLabels(
            ['ID', 'Наименование компании', 'Юридический адрес', 'Номер телефона'])
    ...
    def updateTableProviderOrder(self):
        ...
        self.table_pr_od.setHorizontalHeaderLabels(['Код поставки', 'Поставщик', 'Дата создания', 'Книга', 'Количество книг', 'Стоимость'])
        for i, employee in enumerate(rec):
            for x, info in enumerate(employee):
                item = QTableWidgetItem()
                item.setText(str(info))
    ...
    def updateTableOrder(self):
        ...
        self.table_orders.setHorizontalHeaderLabels(
            ['Код заказа', 'Номер заказа', 'Дата создания', 'Время заказа', 'Клиент', 'Книга', 'Сотрудник'])
    ...
    def updateTableOrder_2(self):
        """
        Обновление таблицы `Заказ`.
        """
        self.table_orders_2.clear()
        rec = self.facade.read_order()
        self.table_orders_2.setColumnCount(7)
        self.table_orders_2.setRowCount(len(rec))
        self.table_orders_2.setHorizontalHeaderLabels(
            ['Код заказа', 'Номер заказа', 'Дата создания', 'Время заказа', 'Клиент', 'Книга', 'Сотрудник'])
        ...
        for i, employee in enumerate(rec):
            for x, info in enumerate(employee):
                ...
                if x == 0:
                    item.setFlags(Qt.ItemIsEnabled)
                self.ui.table_orders_2.setItem(i, x, item)
        logging.log(logging.INFO, 'Таблица "Заказ" обновлена')
    ...
    def new_book(self):
        ...
        year = self.ui.year.dateTime().toString('yyyy')
    ...
    def new_author(self):
        ...
        self.ui.edit_i.clear()
        self.ui.edit_o.clear()
    ...
    def new_provider(self):
        ...
        if name_pr != '' and address_pr != '':
            if phone_pr != '':
                self.facade.insert_provider(name_pr, address_pr, phone_pr)
            if phone_pr == '':
                self.facade.insert_provider(name_pr, address_pr, '')
        self.updateTableProvider()
        self.ui.edit_name_pr.clear()
        self.ui.edit_address_pr.clear()
        self.ui.edit_phone_pr.clear()
    ...
    def get_from_table_provider(self):
        """
        Получение данных из таблицы `Поставщик` для записи в базу данных.
        :return: data
        """
    ...
    def save_provider(self):
        """
        Сохранение данных о поставщиках в базу данных
        """
    ...
    def get_from_table_req(self):
        """
        Получение данных из таблицы `Заказ` для записи в базу данных.
        :return: data
        """
    ...
    def save_req(self):
        """
        Сохранение данных о заказах в базу данных
        """
    ...
    def delete_pr_od(self):
        ...
        if rowcount == 0:
            ...
            logging.log(logging.INFO, 'Удалены данные из таблицы "Поставка"')

    def get_from_table_pr_od(self):
        """
        Получение данных из таблицы `Поставка` для записи в базу данных.
        :return: data
        """
        ...
        for row in range(rows):
            ...
            data.append(tmp)
        return data
        logging.log(logging.INFO, 'Получены данные для сохранения в таблицу "Поставка"')

    def save_pr_od(self):
        """
        Сохранение данных о поставках в базу данных
        """
        ...
        self.updateTableProviderOrder()
        logging.log(logging.INFO, 'Данные сохранены в таблицу "Поставка"')
    ...
    def build_combobox_provider(self):
        """
        Добавление списка поставщиков в ComboBox.
        """
    ...
    def add_book_to_pr_od(self):
        """
        Добавление книги в поставку.
        """
        if self.listWidgetDelivery.count() != 0:
            quantity = self.spinBoxQuantityBook.value()
            if quantity >= 1:
                info_book = str(self.comboBookProvider.currentText()) + ' — ' + str(
                    self.spinBoxQuantityBook.value()) + ' шт.'
    ...
    def order_pdf(self, type, data):
        ...
        if type == 1:
            ...
            logging.log(logging.INFO,
                        'Сгенерирован отчёт "Количество проданных книг по дням за период времени" в pdf-формате')

        elif type == 2:
            pdf.set_font('Calibri Regular', size=14)
            pdf.cell(200, -20, txt="Отчёт по количеству заказов по дням за период времени по каждой книге", ln=1,
                     align="C")
            pdf.set_font('Calibri Regular', size=14)
            pdf.text(x, y, 'Дата')
            ...
            logging.log(logging.INFO,
                        'Сгенерирован отчёт "Количество заказов по дням за период времени по каждой книге" в pdf-формате')
...
class DialogAuth(QDialog):
    def __init__(self, parent=None):
        """
        Подключением к кнопкам, объявление переменных.
        """
    ...
    def vis_pas(self):
        ...
        if self.vis_p:
            ...
            self.ui.btn_hide_password.setIcon(QtGui.QIcon('img/eye2.png'))
            logging.log(logging.INFO, 'Пароль скрыт')
        else:
            self.vis_p = True
            ed.setEchoMode(QtWidgets.QLineEdit.Normal)
            self.ui.btn_hide_password.setIcon(QtGui.QIcon('img/eye.png'))
    ...
    def visible_captcha(self, visible=True):
        ...
        """
        self.ui.captcha.setVisible(visible)
    ...
    def captcha_generation(self):
        ...
        Выводит капчу в «label» и возвращает значение капчи в переменной self.now_captcha.
        """
        syms = 'qwertyuiopasdfghjklzxcvbnm1234567890'
        count_syms = random.randint(5, 10)
        now_syms = [''] * count_syms
        ...
        imagea = ImageCaptcha(width = 407, height = 67)
        data = imagea.generate(self.now_captcha)
        imagea.write(self.now_captcha, 'CAPTCHA.png')
        self.ui.captcha.setPixmap(QPixmap('CAPTCHA.png'))
        ...
        if auth_log == '' or auth_pas == '':
            ...
            if self.now_captcha is not None and self.now_captcha != self.ui.edit_captcha.text():
                ...
                if self.count_try_entry >= 3 and self.next_try > now_time:
                    ...
                    self.parent().ui.lbl_role.setText(f'  {role}')

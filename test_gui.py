...
class Test1Push(TestCase):
    ...
    def test_push_provider(self):
        btn_add = self.window.ui.btn_add_prov

        self.window.ui.edit_name_pr.setText("test_provider")
        self.window.ui.edit_address_pr.setText("test_provider")
        self.window.ui.edit_phone_pr.setText("test_provider")
...
class Test2DeleteAndSave(TestCase):
    ...
    def test_delete_provider(self):
        rowcount = self.window.table_provider.rowCount()
        self.window.table_provider.setCurrentCell(rowcount-1, 1, QItemSelectionModel.SelectionFlag.Select)

        btn_del = self.window.ui.btn_del_prov
        QTest.mouseClick(btn_del, QtCore.Qt.MouseButton.LeftButton)
        QTest.mouseClick(self.window.ui.btn_save_prov, QtCore.Qt.MouseButton.LeftButton)
